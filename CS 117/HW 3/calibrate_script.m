...
avgX = mean(abs(Xtrue(1,:) - Xest(1,:)));
avgY = mean(abs(Xtrue(2,:) - Xest(2,:)));
avgZ = mean(abs(Xtrue(3,:) - Xest(3,:)));
avgError = mean([avgX avgY avgZ]);
...
base = [[lcamo(1) lcamz(1)],[lcamo(2) lcamz(2)],[lcamo(3) lcamz(3)];
      [rcamo(1) rcamz(1)],[rcamo(2) rcamz(2)],[rcamo(3) rcamz(3)] ];
baseline = pdist(base,'euclidean')
