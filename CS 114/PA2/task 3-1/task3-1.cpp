...
struct SpecularBRDF : public BRDF {
    ...
    Vec eval(const Vec &n, const Vec &o, const Vec &i) const {
        Vec dir = mirroredDirection(n,o);
        
        if ((dir.x == i.x) && (dir.y == i.y) && (dir.z == i.z)) {
...
const SpecularBRDF specularSurface(Vec(0.999, 0.999, 0.999));
...
void luminaireSample(const Vec point, double rad, Vec &ny, double &pdf, Vec &out){
    double a = rng();
    double z = 2.0 * a - 1.0;
    
    double b = rng();
    double x = pow(1.0 - (z * z), 0.5) * cos(2.0 * PI * b);
    double y = pow(1.0 - (z * z), 0.5) * sin(2.0 * PI * b);
    
    ny = Vec(x, y, z).normalize();
    out = point + (ny * rad);
...
Vec reflectedRadiance (const Ray &r, int depth) {
    ...
    Vec ny;
    double pdf;
    luminaireSample(objy.p, objy.rad, ny, pdf, out);
    
    Vec w1 = (out - x).normalize();
    
    if ( ny.dot(w1 * (-1.0)) < 0) {
        ny = ny * (-1.0);
    }
    
    double radi = (out - x).dot(out - x);
    
    int id2 = 0;
    int id3 = 0;
    intersect(Ray(out, w1 * (-1.0)), t, id2);
    ...
    Vec relflectRad = Le.mult(brdf.eval(n, o, w1)) * scalar1;
    ...
    if (rng() < p){
        ...
        relflectRad = relflectRad + (brdf.eval(n, o, w2).mult(reflectedRadiance(rr2, depth + 1)) * scalar2);
    }
